...
public:
  readData(): creationApproach(NONE), numDataSets(-1){}
  ...
  bool loadData(const char* filename, int nI, int nT){
    for(int i=0;i<(int) data.size(); ++i) delete data[i];
  ...
  void setCreationApproach(int approach, double param1 = -1, double param2 = -1){
	...
	numDataSets = (int) ceil((double) (trainingDataEndIndex - windowingSetSize)/ windowingStepSize);
      }
    }
  }
...
private:
  ...
  void createGrowingDataSet(){
    ...
    if(growingLastDataIndex > (int) trainingDataIndex) growingLastDataIndex = trainingDataLastIndex;
  ...
  void createWindowingDataSet(){
    ...
    if(endIndex > trainingDataLastIndex) endIndex = trainingDataLastIndex;
